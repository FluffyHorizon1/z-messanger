...
| The double‑ratchet state with your own other devices, and with each contact's other devices | `kv` keys `sync_session` and `cextra_<rid>`, **sealed** | You, once the OS user and any passphrase are unlocked | Until the session is replaced. Until 2026‑09‑14 both opted out of sealing and held the root key, the ratchet seed, both chain keys and the cached skipped message keys in the clear; a vault that still does is sealed when it opens. Until the same date the contact half was written to a key whose name contained a literal `$rid` rather than the routing id, so it was never read back and the session was rebuilt from the contact's device certificates at every launch; one such row is deleted when a vault opens |
| Which key‑value cells may be stored unsealed at all | `Vault.plainKeys` and `Vault.plainPrefixes` — relay URL, device id, developer mode, push flag, backup timings, this account's own signed device list and its alerts, the transparency log's heads and faults, and the per‑contact families keyed by routing id | Anyone who reads `z.db` — which is why the list is short, declared, and enforced by `kvPut` rather than decided at each call site | Life of the install |
| Group membership and metadata | `kv` key `groups`, sealed | Same | Until you leave or delete the group |
| Device-list warnings raised about a contact (their devices disagree; a list went backwards; the post-quantum signature never arrived) | `kv` keys `cdl_alert_<rid>`, `pql_alert_<rid>` — the KIND of warning only, unsealed, like the device-list versions beside it | You, once the OS user and any passphrase are unlocked | Until the condition clears or you dismiss it. Until 2026‑09‑13 these held the finished English SENTENCE, each of which names the contact, so a display name sat here in the clear beside the routing id that says whose it is. The name is not stored at all now: the screen has the contact and supplies it when it draws the banner |
| Who has confirmed each outgoing message, and when | `delivery` — one row per (message, confirmer); the ids are the routing ids already in `contacts` and `messages` | You, once the OS user and any passphrase are unlocked | Deleted with the message it is about — including when a disappearing-message timer fires, so nothing outlives the message to say who received it |
...
| An attachment being restored from an archive, while the restore runs | `restore-spill/` under the vault root — each archive frame re‑sealed as it arrives, under a key drawn for that restore and held only in the process; never the attachment's bytes | Nobody: the key is gone when the process is, whether the restore finished or the OS killed it | Removed, zeroed first, when the restore returns; and whatever a dead process left is removed when the vault next opens. Until 2026‑09‑17 this was plaintext, removed only on a clean exit, and collected by nothing — the one exit the spill exists for (the OS killing the app for memory mid‑restore) left every attachment restored so far in the clear for the life of the install |
...
| A contact and its history | Delete contact | Nothing locally. Their copy is theirs. Until 2026‑09‑17 this row was false: the tables went and every `kv` family keyed by their routing id stayed — their account key, their devices' public keys with the list's version and signature, the post‑quantum material, the ratchet with their linked devices, the alerts and what the log said — unsealed wherever the family is declared plain, for the life of the install, and a contact added back met their own stale list version. The families are one declared list now (`ChatService.contactKvFamilies`), removed together with the rows, and `contact_erasure_test.dart` searches the whole vault for the routing id afterwards. It was still not quite true until 2026‑09‑20: the deletion took no lock, so a send already in flight — sealed and one step from its transaction — could commit INTO the sweep and leave the envelope, the message row or a fresh conversation behind, which is what failed the 3.9.0 release build. The deletion now runs under the same per‑contact lock a send holds, and a send that was queued behind it writes nothing |
...
(schema 11) and the relay in `server/server.js`. If a column is added to either
