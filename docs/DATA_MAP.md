...
| Ratchet state (chain keys, skipped message keys) | `conversations`, sealed | Same | Rolling; old keys are dropped as chains advance |
...
(schema 8) and the relay in `server/server.js`. If a column is added to either
