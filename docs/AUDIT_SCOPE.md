...
| C25 | The shipped binary can be checked against the source: two independent release builds of the same commit are byte-for-byte identical, signature included, under the toolchain pinned in `REPRODUCIBLE_BUILDS.md`. The app carries no encrypted third-party dependency-metadata blob. A verifier is given a tool that reports precisely what differs — entry, timestamp, CRC, content, and which signing-block pair — rather than a bare pass/fail. Cross-machine reproducibility is NOT yet established and the document says so. | `REPRODUCIBLE_BUILDS.md` | `tool/verify_reproducible.py`, exercised on both an identical and a differing pair |
