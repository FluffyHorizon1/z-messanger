...
| `app/` | Flutter (Dart), core ~11.0 k lines (`lib/core/`), UI ~6.4 k lines, 54 test files (~14.7 k lines), most driving real clients through a real relay | The orchestrator: encrypted vault, outbox, session/ratchet persistence, multi‑device self‑sync, groups fan‑out, transparency alerts, voice, search, history sync |
...
# App: 54 test files, most driving real clients through the real relay
