...
| C25 | **Partly false as of 2026-09-09, and left stated so.** On one machine at one path, two release builds of the same commit are byte-for-byte identical, signature included, under the toolchain pinned in `REPRODUCIBLE_BUILDS.md` (measured, 14.1). **Across two CI runners the APK is NOT reproducible**: the first real run of the cross-machine job failed, with all six `libapp.so`/`libdartjni.so` entries differing and every other entry identical. The app carries no encrypted third-party dependency-metadata blob. A verifier is given a tool that reports precisely what differs — entry, timestamp, CRC, content, and which signing-block pair. **A reviewer's most useful contribution to this row is the cause**: see `REPRODUCIBLE_BUILDS.md` §'What the first cross-machine run found'. | `REPRODUCIBLE_BUILDS.md`, `PROVENANCE.md` | `tool/verify_reproducible.py`, exercised on an identical and a differing pair; CI run 34376105061, which is the evidence AGAINST the cross-machine half of this claim |
