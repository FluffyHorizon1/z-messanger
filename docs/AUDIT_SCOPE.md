...
| `app/` | Flutter (Dart), core ~8.4 k lines (`lib/core/`), UI ~6.0 k lines, 28 test files (~7.6 k lines), most driving real clients through a real relay | The orchestrator: encrypted vault, outbox, session/ratchet persistence, multi‑device self‑sync, groups fan‑out, transparency alerts, voice, search, history sync |
...
# App: 28 test files, most driving real clients through the real relay
