...
| `app/` | Flutter (Dart), core ~10.7 k lines (`lib/core/`), UI ~6.1 k lines, 46 test files (~12.2 k lines), most driving real clients through a real relay | The orchestrator: encrypted vault, outbox, session/ratchet persistence, multi‑device self‑sync, groups fan‑out, transparency alerts, voice, search, history sync |
...
# App: 46 test files, most driving real clients through the real relay
