...
| `app/` | Flutter (Dart), core ~10.5 k lines (`lib/core/`), UI ~5.9 k lines, 45 test files (~11.8 k lines), most driving real clients through a real relay | The orchestrator: encrypted vault, outbox, session/ratchet persistence, multi‑device self‑sync, groups fan‑out, transparency alerts, voice, search, history sync |
...
# App: 45 test files, most driving real clients through the real relay
