...
| `app/` | Flutter (Dart), core ~11.0 k lines (`lib/core/`), UI ~6.4 k lines, 56 test files (~15.2 k lines), most driving real clients through a real relay | The orchestrator: encrypted vault, outbox, session/ratchet persistence, multi‑device self‑sync, groups fan‑out, transparency alerts, voice, search, history sync |
...
# App: 56 test files, most driving real clients through the real relay
