...
| R35 | **The update check tells the relay's host that an install looked for a newer build.** 24.4 adds an HTTPS GET to `/latest.json` on the relay's own host: it reveals that install's address and a timestamp to that host, at most once a day, when Settings is opened. | The relay host — which already sees the client's WebSocket connection from the same address; anyone on the path to a cleartext (`ws`) relay | Telling a user they are running an old build (R8) means fetching a "latest" from *some* host. Pointing the check at the relay the client already dials means it is a host that already sees this address, not a new party (a GitHub-hosted check would have handed Microsoft/GitHub every install's IP on a schedule). It cannot be removed without removing the notice | The request is **identity-free**: a plain GET with no account, no routing id, no cookie and no body — the same `/latest.json` every install gets, so it singles nobody out. It goes only to the relay's own host, derived from the dialled address (`update_check.dart`), so no third party is contacted. It is **user-triggered** (opening Settings) and **rate-limited to once per 24 h** — not a background beacon (R30's shape, deliberately not repeated). A self-hosted relay that leaves `Z_LATEST_VERSION` unset serves 404 and the client shows nothing, so the check reaches only infrastructure the user already chose. And there is no in-app updater: the notice links to a download, nothing is fetched or run (R8) (`update_check_test.dart`, `server/test/pages.test.js`) | **accepted** — `adr/0014` |
| R36 | **A video (or voice note) being played is reachable on a localhost port for as long as it plays.** Decrypted media is handed to the player over a loopback HTTP socket rather than a file — the voice notes through `just_audio`'s own server, a video (continuous‑b) through the app's (`loopback_media.dart`) — and on Android any app on the device can open a connection to `127.0.0.1`. | Other apps on the same device, for the seconds a bubble is playing | A player wants a URL or a file; a file is plaintext on disk (C11), so the bytes stay in memory and are served from there. The socket has to be reachable by the player process, which is this app, but the OS does not scope loopback ports to a package | The socket is bound to the loopback interface only, on a random port, and serves one random 32‑byte path — a request for anything else is a bare 404, so the port alone yields nothing; it lives exactly as long as the bubble and is closed on dispose and on any failure; the body is marked `no-store`; and a request from a non‑loopback address is refused. What is exposed is what is on the screen at that moment, to a process that could also record the screen — nothing that is not already being shown (`loopback_media_test.dart`, `video_bubble_test.dart`) | **accepted** — `adr/0018` |
