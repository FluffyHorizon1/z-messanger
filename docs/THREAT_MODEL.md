...
| R7 | No public key transparency yet: an account's device lists are cross‑checked by its contacts (§3.6) but recorded nowhere a contact who was never told could consult | Anyone whose contact is shown a split view they are not part of | `adr/0006` builds the log — a separate service (`kt/`), Merkle log and map, signed heads, an independent mirror as witness. It is built and vectored; it is not yet live, and the client does not yet read it. A log records what an account published; an exchange the attacker fully controlled from the start it cannot detect, which only safety numbers catch | Safety numbers, which catch a substituted exchange the moment two people compare; gossip (§3.6) for everything after | **deferred** — live when the four conditions in `adr/0006` "what live means" hold; this series lands the client |
