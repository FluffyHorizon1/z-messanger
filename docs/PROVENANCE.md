...
| The upload of those artefacts to the release | **not exercised** — `dry_run_release.py` reads only `run:` steps, and every upload is a `uses:` step, so nothing rehearses the uploads themselves. Four tagged releases have failed in exactly that gap. What is checked is the result: the release job now publishes nothing, `release-verify` requires every file `SHA256SUMS.txt` lists to be on the draft, `uploaded` and non-empty, and `release-publish` flips the draft only if it is. A release that lost a file is never public |
| The `release-verify` job's own shell | **exercised** — `tool/test_release_verify.py` pulls that step's body out of `build.yml` so there is one copy, stubs `gh` with fixtures, and runs it against nine shapes including 3.5.3's real asset table. It is outside `jobs.release` on purpose (a `gh` call there would be executed by `dry_run_release.py` on every push), which is what put it beyond that guard's reach and made this one necessary |
